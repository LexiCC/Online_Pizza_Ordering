...
router.get('/', async (req, res) =>{
  ...
  //-------- Customize
  let queryC = 'SELECT * FROM customizations';
  const paramsC = [];
  const resultC = await db.query(queryC, paramsC);

  res.render('index', {customer: req.session.user, title:'Pizza 330', rows: result.rows, rowsC: resultC.rows});
});
  ...
  if(req.body.customizations){
    itemToAdd.customizations = [];
     for(i =0; i < req.body.customizations.length; i++){
      let query2 = 'SELECT * FROM customizations WHERE id = $1';
